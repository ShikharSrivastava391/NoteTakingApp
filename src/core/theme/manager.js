...
const WS_CONFIG_REL = ".NoteMakingApp/config.json";
...
// NoteMakingApp Dark is the default theme and should be first in the list
const DEFAULT_THEMES = ["NoteMakingApp-dark", "dracula", "nord", "one-dark-pro", "minimal-light", "neon-dark"];
const DEFAULT_THEME_CONTENT = {
  "NoteMakingApp-dark": `{"name": "NoteMakingApp Dark", "tokens": {"--bg": "#0f172a", "--text": "#f1f5f9", "--panel": "#1e293b", "--border": "#334155", "--muted": "#94a3b8", "--accent": "#8b5cf6", "--accent-fg": "#ffffff", "--task-todo": "#64748b", "--task-progress": "#3b82f6", "--task-urgent": "#ef4444", "--task-question": "#f59e0b", "--task-completed": "#10b981", "--task-cancelled": "#64748b", "--task-delegated": "#8b5cf6", "--danger": "#ef4444", "--success": "#10b981", "--warning": "#f59e0b", "--info": "#3b82f6", "--editor-placeholder": "#64748b"}}`,
...
export async function loadThemeManifestById(id) {
  ...
  // Otherwise, try to load from Rust backend (custom themes in ~/.NoteMakingApp/themes/)
  if (isTauri) {
    try {
...
export async function listAvailableThemes() {
  ...
  // Add custom themes from Rust backend (reads from ~/.NoteMakingApp/themes/)
  if (isTauri) {
    try {
...
export async function loadThemeForWorkspace(workspacePath) {
  ...
  // Default to NoteMakingApp Dark if no theme is set
  if (!id) {
    id = 'NoteMakingApp-dark';
  }
  const manifest = await loadThemeManifestById(id);
  let tokensToApply = manifest?.tokens || BUILT_IN_THEME_TOKENS;
...
export async function applyInitialTheme() {
  ...
  if (theme === 'system' || theme === 'auto') {
    ...
    if (cleanup) {
      if (window.__NoteMakingAppThemeCleanup) {
        window.__NoteMakingAppThemeCleanup();
      }
      window.__NoteMakingAppThemeCleanup = cleanup;
    }
  }
}
