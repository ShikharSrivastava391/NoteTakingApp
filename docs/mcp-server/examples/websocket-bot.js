 ...
 * 
 * This example demonstrates how to build a chat bot that connects to NoteMakingApp MCP server
...
class NoteMakingAppBot {
  ...
  async initialize() {
    const response = await this.sendRequest('initialize', {
      ...
      clientInfo: {
        name: 'NoteMakingApp Bot',
        version: '1.0.0',
        description: 'AI assistant for NoteMakingApp workspace'
      }
    });
...
*Summary generated by NoteMakingApp Bot*`;
  }
  ...
  async handleGeneralQuery(input) {
    ...
    return `🤖 I'm here to help you with your NoteMakingApp workspace! I can:
...
What would you like me to help you with?`;
  ...
  startInteractiveSession() {
    ...
    console.log(`
🤖 **NoteMakingApp Bot is ready!**

I'm connected to your NoteMakingApp workspace and ready to help. Here are some things I can do:
...
`);
      ...
      try {
        console.log('\n🤖 NoteMakingApp Bot: Processing your request...\n');
        const response = await this.processUserInput(trimmedInput);
        console.log(`🤖 NoteMakingApp Bot: ${response}\n`);
      } catch (error) {
...
// Main execution
async function main() {
  const mcpUrl = process.env.NoteMakingApp_MCP_URL || 'ws://localhost:3001/mcp';
  const apiKey = process.env.NoteMakingApp_API_KEY;

  if (!apiKey) {
    console.error('❌ Please set NoteMakingApp_API_KEY environment variable');
    process.exit(1);
  }

  const bot = new NoteMakingAppBot(mcpUrl, apiKey);
...
module.exports = NoteMakingAppBot;